...
{
    /// <summary>
    /// A specialization of the OutOfProcessNodeInstance base class that uses a lightweight binary streaming protocol
    ...
    internal class SocketNodeInstance : OutOfProcessNodeInstance
    {
        ...
        private readonly SemaphoreSlim _connectionCreationSemaphore;
        ...
        protected override async Task<T> InvokeExportAsync<T>(NodeInvocationInfo invocationInfo, CancellationToken cancellationToken)
        {
            ...
            if (_virtualConnectionClient == null)
            {
                ...
                await ThrowOnCancellation(EnsureVirtualConnectionClientCreatedAsync(), cancellationToken);
            }
            ...
            var shouldDisposeVirtualConnection = true;
            ...
            try
            {
                ...
                else
                {
                    ...
                    var response = await ReadJsonAsync<RpcJsonResponse<T>>(virtualConnection, cancellationToken).ConfigureAwait(false);
        ...
        private async Task EnsureVirtualConnectionClientCreatedAsync()
        {
            // Asynchronous equivalent to a 'lock(...) { ... }'
            await _connectionCreationSemaphore.WaitAsync().ConfigureAwait(false);
            try
            {
                if (_virtualConnectionClient == null)
                {
                    ...
                    var connection = await _physicalConnection.Open(_socketAddress).ConfigureAwait(false);
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                _connectionCreationSemaphore?.Dispose();
            }
