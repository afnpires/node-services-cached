...
namespace Microsoft.AspNetCore.NodeServices.Sockets.VirtualConnections
{
    ...
    {
        private const int MaxConcurrentRequestCount = 1;
        internal const int MaxFrameBodyLength = 16 * 1024;

        public event VirtualConnectionReadErrorHandler OnError;

        private readonly Stream _underlyingTransport;
        private readonly Dictionary<long, VirtualConnection> _activeInnerStreams;
        private long _nextInnerStreamId;
        private readonly SemaphoreSlim _streamWriterSemaphore;
        private Exception _readLoopExitedWithException;
        private readonly CancellationTokenSource _disposalCancellatonToken;
        private bool _disposedValue = false;

        public VirtualConnectionClient(Stream underlyingTransport)
        {
            _streamWriterSemaphore = new SemaphoreSlim(MaxConcurrentRequestCount);
            _disposalCancellatonToken = new CancellationTokenSource();
        ...
        // It's async void because nothing waits for it to finish (it continues indefinitely).
        // It signals any errors via a separate channel.
        private async void RunReadLoopAsync()
        {
            try
            {
        ...
        private async Task<bool> ProcessNextFrameAsync()
        {
            ...
            var frameHeaderBuffer = await ReadExactLengthAsync(12);
            ...
            var streamId = BitConverter.ToInt64(frameHeaderBuffer, 0);
            ...
            var frameBody = await ReadExactLengthAsync(frameBodyLength).ConfigureAwait(false);
            if (frameBody == null)
            {
                return false; // Underlying stream was closed
            }
            ...
            if (innerStream != null)
            {
                await innerStream.AddDataToQueueAsync(frameBody).ConfigureAwait(false);
            }

            return true;
        }

        private async Task<byte[]> ReadExactLengthAsync(int lengthToRead)
        {
        ...
        private void Dispose(bool disposing)
        {
            if (disposing && !_disposedValue)
            {
                ...
                _disposalCancellatonToken.Dispose();
                CloseAllActiveStreams();
                _streamWriterSemaphore.Dispose();
            }
        }
        ...
        public void CloseInnerStream(long innerStreamId, bool isAlreadyClosedRemotely)
        {
            ...
            {
                // Also notify the remote that this inner-stream is closed
                WriteAsync(innerStreamId, Array.Empty<byte>(), 0, 0, new CancellationToken()).Wait();
            }
        }
