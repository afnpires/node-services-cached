...
namespace Microsoft.AspNetCore.NodeServices.Sockets.VirtualConnections
{
    ...
    internal class VirtualConnection : Stream
    {
        ...
        private readonly VirtualConnectionClient _host;
        private readonly BufferBlock<byte[]> _receivedDataQueue;
        ...
        public VirtualConnection(long id, VirtualConnectionClient host)
        {
            Id = id;
            _host = host;
            _receivedDataQueue = new BufferBlock<byte[]>();
        }
        ...
        public override async Task<int> ReadAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                ...
                var bytesToExtract = Math.Min(count - bytesRead, _receivedDataNotYetUsed.Count);
                if (bytesToExtract > 0)
                {
                ...
                else
                {
                    ...
                    nextReceivedBlock = await _receivedDataQueue.ReceiveAsync(cancellationToken).ConfigureAwait(false);
                }
        ...
        public async Task AddDataToQueueAsync(byte[] data)
        {
